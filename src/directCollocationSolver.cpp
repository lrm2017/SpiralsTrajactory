...
//    cout << "pv:" << pVec.size() <<endl;
    ...
    opt.subject_to(X(Slice(0,3),0) == start.state(Slice(0,3)));
    ...
    for (casadi_int k = 0; k< N; ++k) {
        ...
        opt.subject_to( (Xc(0)-5)*(Xc(0)-5)+(Xc(1)-5)*(Xc(1)-5) >= 1 );
        ...
        if(k == N-1){
            ...
            opt.subject_to( (X(0, k+1)-5)*(X(0, k+1)-5)+(X(1, k+1)-5)*(X(1, k+1)-5) >= 1 );
            opt.set_initial(X(Slice(), k+1), DM::zeros(SSZ));
        }
        opt.subject_to(Xk_end == X(Slice(), k+1));
    }

    opt.subject_to(Xk_end(Slice(0,3)) == goal.state(Slice(0,3))); // 末端约束
