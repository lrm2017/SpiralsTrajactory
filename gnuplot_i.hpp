...
class Gnuplot
{
    ...
    public:
        ...
        Gnuplot(const char *style = "points");
    ...
    //----------------------------------------------------------------------------------
    Gnuplot& set_font(const std::string f, const int fs ){
        font = f;
        fontsize = fs;
    }
    ...
    Gnuplot& savePic(const std::string &filename = "gnuplot_output", const std::string type="png", int lw=2);
    ...
    Gnuplot& set_label(double x, double y, const std::string label="", const std::string pos="left", const int pt=7, const int ps=2, const std::string lc="");
    Gnuplot& set_label_points(double x, double y,  const int pt=7, const int ps=2, const std::string lc="");
    Gnuplot& set_lineColor(const std::string &colorstr);
    Gnuplot& select_lineColor(const int c);
    Gnuplot& set_lineWidth(const int lw){ lineWidth=lw; return *this;};

    Gnuplot& set_view(const int xang, const int zang){
        std::ostringstream cmdstr;
        cmdstr << "set view " << xang << "," << zang << "\nreplot";
        cmd(cmdstr.str());
        return *this;
    }
    ...
    /// turns grid on/off
    inline Gnuplot& set_grid()	{cmd("set grid\n");return *this;};
    /// grid is not set by default	
    inline Gnuplot& unset_grid(){cmd("unset grid\n");return *this;};

    inline Gnuplot& set_dgrid3d(const int m, const int n){
        std::ostringstream cmdstr;
        cmdstr << "set dgrid3d " << m << "," << n;
        cmd(cmdstr.str());

        return *this;
    };

//    inline Gnuplot& set_hi
    ...
    inline Gnuplot& set_splot(){ two_dim=true; return *this;};
    ...
    /// \return <-- reference to the gnuplot object
    // --------------------------------------------------------------------------
    inline Gnuplot& set_hidden3d(){cmd("set hidden3d\n");return *this;};
...
//    Gnuplot
    ...
    inline Gnuplot& set_pm3d(){cmd("set pm3d\n");return *this;}
...
//
inline Gnuplot::Gnuplot(const char *style)
...
Gnuplot& Gnuplot::set_label_points( double x, double y, const int pt, const int ps, const std::string lc) {
    std::ostringstream cmdstr;
    cmdstr << "set label " << labels++ << " at " << x <<"," << y << " "
           << " point pointtype " << pt << " pointsize " << ps ;
    if( lc != ""){
        cmdstr << " lc rgb \"" << lc << "\"";
    }
    cmdstr << "\n";
...
Gnuplot& Gnuplot::set_label( double x, double y, const std::string label, const std::string pos, const int pt, const int ps, const std::string lc) {
    std::ostringstream cmdstr;
    cmdstr << "set label " << labels++ << " at " << x <<"," << y << " "
    << pos << " \"" << label << " \" font \""<< font << "," << fontsize << "\"";
    if( lc != ""){
        cmdstr << " lc rgb \"" << lc << "\"";
    }
    cmdstr << "\n";
...
Gnuplot& Gnuplot::set_lineColor(const std::string &colorstr) {
    lcolors = colorstr;
    return *this;
}

Gnuplot& Gnuplot::select_lineColor(const int c){
    colortype = c;
    return *this;
}
...
Gnuplot& Gnuplot::savePic(const std::string &filename, const std::string type, int lw){
    std::ostringstream cmdstr;
    cmdstr << "set term " << type << "cairo " << "size 1200,800 " << "lw " << lw <<  " font \""<< font << "," << fontsize << "\""<< "\n";
    cmdstr << "set output \"" << filename << "." << type << "\"\n";
    cmd(cmdstr.str());
}
                               ...
                               const std::string &title)
{
    ...
    // command to be sent to gnuplot
    //
    cmdstr << (nplots > 0 && two_dim == false ? "replot " : "splot ");
